...
public:
    ...
    void init(int rowNumber=2)
    {
        bool isInPanel = Parent->parent() && Parent->parent()->inherits("SARibbonPannel");
        if (isInPanel) {
            QGridLayout *layout = new QGridLayout(Parent);
            layout->setDefaultPositioning(rowNumber, Qt::Vertical); // 控制组的行数
...
SARibbonButtonGroupWidget::SARibbonButtonGroupWidget(QWidget *parent, int rowNumber)
    : QFrame(parent)

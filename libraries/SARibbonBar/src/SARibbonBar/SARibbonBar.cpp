...
public:
    ...
    void init()
    {
        ...
        stackedContainerWidget->setObjectName(QStringLiteral("objSAStackedContainerWidget"));
 ...
 * @param parent
 */
SARibbonBar::SARibbonBar(QWidget* parent) : QMenuBar(parent), m_pWidget(parent), m_d(new SARibbonBarPrivate(this))
{
    m_d->init();
    if (parent) {
...
void SARibbonBar::mouseDoubleClickEvent(QMouseEvent* event)
{
    if (Qt::LeftButton == event->button())
    {
        //QWidget* parentWidget = m_pWidget->parentWidget();
        QPoint pt = event->pos();
        double height = m_d->quickAccessBar->height();
        if (pt.y() < height)
        {

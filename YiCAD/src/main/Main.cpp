...
const char* const REG_KEY_THEME = "/Theme";
...
int App_Run(int argc, char* argv[])
{
    ...
    DMSETTINGS->beginGroup(REG_GROUP_DEFAULTS);
    bool m_theme = DMSETTINGS->readNumEntry(REG_KEY_THEME, 0) != 0;
    DMSETTINGS->endGroup();
    ...
    if (m_theme)
    {
        w.setRibbonTheme(SARibbonMainWindow::DarkTheme);
    }
    else
    {
        w.setRibbonTheme(SARibbonMainWindow::LightTheme);
    }

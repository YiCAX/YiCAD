...
void UIDlgOptionsGeneral::init()
{
    ...
    choTheme->setCurrentIndex(DMSETTINGS->readNumEntry("/Theme", 0));
...
{
    int theme_index = choTheme->currentIndex();
    ...
    if (theme_index != 0)
    {
        appWindow->setRibbonTheme(SARibbonMainWindow::DarkTheme);
    }
    else
    {
        appWindow->setRibbonTheme(SARibbonMainWindow::LightTheme);
    }

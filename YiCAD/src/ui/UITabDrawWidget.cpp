...
QWidget* UITabDrawWidget::createTabDrawWidget(QMdiArea* drawBackWidget, UIActionHandler* pActionHandler, UIBottomWindow* pBottomWidget)
{
	...
	newDraw->resize(25, 25);
	newDraw->setAutoFillBackground(true);
	...
	// docView绘图区域的当前画笔栏
	QWidget* pPenWidget = new QWidget(m_pWidget);
	pPenWidget->move(1, 186);
	pPenWidget->resize(700, 28);
 	m_pCurrentActivePen = new UICurrentActivePen(pPenWidget);
...
void UITabDrawWidget::newTabDraw(SingleTabDrawDataRibbon* newTab)
{
	...
	drawWidget->resize(90, 25);
	drawWidget->move(m_pTabDrawList->size() * 90 + 25, 0);
	...
	connect(closeDraw, &QPushButton::clicked, newTab->tabBack, [this, newTab, drawWidget, closeDraw] {
		...
		closeTab(newTab);
		double num = m_pTabDrawList->size();
		double len = m_pBackTabDrawWidget->width() - 25;
		double unitLen = len / num;
		for (int i = 0; i < m_pTabDrawList->size(); ++i)
		{
			SingleTabDrawDataRibbon* tabData = (*m_pTabDrawList)[i];
			unitLen = unitLen > 90 ? 90 : unitLen;
			tabData->theWidget->resize(unitLen, 25);
			tabData->theWidget->move(i * unitLen + 25, 0);
			if (m_currentBtnWidget == tabData->theWidget)
			{
				m_currentPos = tabData->theWidget->pos();
			}
			tabData->textBtn->resize(unitLen / 9 * 7, 25);
			tabData->closeDraw->resize(unitLen / 9 * 1.5, 15);
			tabData->closeDraw->move(unitLen / 9 * 7, 5);
		}
	});
	...
	m_btnIndex = (*m_pTabDrawList).size() - 1;

	// 设置tab控件位置
	double num = m_pTabDrawList->size();
	if (num * 90 + 25 > m_pBackTabDrawWidget->width())
	{
		for (int i = 0; i < m_pTabDrawList->size(); ++i)
		{
			SingleTabDrawDataRibbon* tabData = (*m_pTabDrawList)[i];

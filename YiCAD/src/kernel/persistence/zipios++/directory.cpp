...
struct std::filesystem::dir_it::representation
{
	representation():
		m_handle(nullptr),
		m_refcount(1),
...
std::filesystem::dir_it::dir_it():
	rep(new representation())
{
}

std::filesystem::dir_it::dir_it(std::string const &dirname):
	rep(new representation(dirname))
{
}

std::filesystem::dir_it::dir_it(std::filesystem::dir_it const &it):
...
std::filesystem::dir_it &std::filesystem::dir_it::operator= (std::filesystem::dir_it const &it)
{
	it.rep->reference();
	delete rep->release();
	rep = it.rep;
...
std::string std::filesystem::dir_it::operator* () const
{
	return *(*rep);
}

std::filesystem::dir_it &std::filesystem::dir_it::operator++ ()
{
	++(*rep);
	return *this;
}

std::filesystem::dir_it::proxy std::filesystem::dir_it::operator++ (int)
{
	std::string rc(*(*rep));
	++(*rep);
	return rc;
...
bool std::filesystem::dir_it::operator== (std::filesystem::dir_it const &it) const
{
	return *rep == *(it.rep);
}
